...
def test_project_structure():
    ...
    assert os.path.exists(os.path.join(PROJECT_ROOT, 'src')), "Source directory missing"
    assert os.path.exists(os.path.join(PROJECT_ROOT, 'tests')), "Tests directory missing"
    assert os.path.exists(os.path.join(PROJECT_ROOT, 'requirements.txt')), "Requirements file missing"
...
def test_python_version():
    ...
    assert (major, minor) >= (3, 8), f"Python version {major}.{minor} is not supported"
