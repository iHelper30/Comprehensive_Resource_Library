...
class MetadataEnricher:
    ...
    def extract_readme_summary(self, folder_path: str) -> str:
    ...
    def generate_folder_metadata(self, folder_name: str) -> Dict[str, Any]:
        ...
        categories = {
            '01-10': 'Introduction and Fundamentals',
            '11-20': 'Advanced Strategies',
            '21-30': 'Professional Development',
            '31-40': 'Tools and Resources',
            '41-50': 'Advanced Topics'
        }

        # Assign category based on numeric prefix
        category = next(
            (cat for range_key, cat in categories.items() 
             if self._in_range(prefix, range_key)), 
            'Uncategorized'
        )
        ...
        metadata = {
            ...
            'category': category,
            'summary': self.extract_readme_summary(folder_path),
            'tags': [word.lower() for word in title.split('_')],
            'content_files': [f for f in os.listdir(folder_path) if f.endswith(('.md', '.txt', '.html'))],
            'difficulty': self._determine_difficulty(folder_name)
        }

        return metadata

    def _in_range(self, prefix: str, range_key: str) -> bool:
        """Check if prefix is in the specified range"""
        start, end = map(int, range_key.split('-'))
        return start <= int(prefix) <= end

    def _determine_difficulty(self, folder_name: str) -> str:
        ...
        return next(
            (diff for range_key, diff in difficulty_map.items() 
             if self._in_range(prefix, range_key)), 
            'Unspecified'
        )

    def enrich_library_metadata(self) -> Dict[str, Any]:
...
def main():
    ...
    # Generate and save library metadata
